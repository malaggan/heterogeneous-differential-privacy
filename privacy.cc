...
std::pair<double, double> user::
    ...
    switch (pc) {
        case privacy_class::BLIND:
            return std::make_pair(0.0, 0.0);// FIXME exactly 0
        case privacy_class::CONCERNED:
            return std::make_pair(0.0, 1.0);// FIXME 0,   0.5 , or 1
        case privacy_class::NORMAL:
            return std::make_pair(0.5, 1.0);// FIXME 0.5, 0.75, or 1
        case privacy_class::NAIVE:
            return std::make_pair(0.5, 0.5);// FIXME exactly 0.5
        case privacy_class::UNCONCERNED:
            return std::make_pair(1.0, 1.0);// FIXME exactly 1
        case privacy_class::SLICES: {
            assert(vm.count("min"));
            auto min = vm["min"].as<double>();
            assert(0.0 <= min && min <= 1.0);
            return std::make_pair(min, 1.0);
        }
        case privacy_class::BASELINE:
            assert(false);// if not private, there is no point calling this.
        default:
            assert(false);
    }
    assert(false);
