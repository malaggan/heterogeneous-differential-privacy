...
//------------------------------------------------------------
enum class dataset_t { survey = 1,
                       digg,
                       delicious };
dataset_t to_dataset(std::string str) {
    if (str == "survey.txt") return dataset_t::survey;
    else if (str == "digg.txt")
        return dataset_t::digg;
    else if (str == "delicious.txt")
        return dataset_t::delicious;
    else
        assert(false);
}
std::ostream &operator<<(std::ostream &os, const dataset_t &d) {
    switch (d) {
        case dataset_t::survey:
            os << "survey";
            break;
        case dataset_t::digg:
            os << "digg";
            break;
        case dataset_t::delicious:
            os << "delicious";
            break;
        default:
            assert(false);
...
expr_t to_expr(std::string const &str) {
    if (str == "baseline") return expr_t::baseline;
        ...
        return expr_t::slices;
    else if (str == "groups")
        return expr_t::groups;
    else if (str == "naive")
        return expr_t::naive;
    else
        assert(false);
