...
void output_min_plot(std::unordered_map<std::tuple<dataset_t, min_t>, acc> const &min_expr_values,
    ...
    for (auto &dataset : vector<dataset_t>{dataset_t::survey, dataset_t::digg, dataset_t::delicious}) {
        ...
        tie(baseline_mean, ignore) = mean_range(baseline.at(dataset));
        double random_mean;
        tie(random_mean, ignore) = mean_range(random.at(dataset));
    ...
    for (auto &dataset : vector<dataset_t>{dataset_t::survey, dataset_t::digg, dataset_t::delicious}) {
        for (auto &norm : vector<norm_t>{norm_t::_10, norm_t::_20, norm_t::_60, norm_t::_70}) {
            for (auto &ptype : vector<peer_type>{peer_type::concerned, peer_type::normal, peer_type::unconcerned}) {
                ...
                switch (norm) {
                    case (norm_t::_10):
                        concs = vector<conc_t>{conc_t::_80, conc_t::_70, conc_t::_30, conc_t::_20};
                        break;
                    case (norm_t::_20):
                        concs = vector<conc_t>{conc_t::_70, conc_t::_60, conc_t::_20};
                        break;
                    case (norm_t::_60):
                        concs = vector<conc_t>{conc_t::_30, conc_t::_20};
                        break;
                    case (norm_t::_70):
                        concs = vector<conc_t>{conc_t::_20};
                        break;
                    default:
                        assert(false);
                }
                for (auto &conc : concs) {
                    ...
                    double mean;
                    tie(mean, ignore) = mean_range(acc);
                    tf % mean;
                }
            }
        }
...
int main() {
    ...
    while (getline(&str, &n, is) >= 0) {
        ...
        switch (expr) {
            case expr_t::baseline:
                baseline[dataset](recall);
                break;
            case expr_t::blind:
                random[dataset](recall);
                break;
            case expr_t::naive: {
