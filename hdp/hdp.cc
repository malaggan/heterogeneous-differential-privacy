        ...
        if (vm["append"].as<bool>())
            ...
            if (!vm["overwrite"].as<bool>()) {
                ...
                cout << "File \"" << fname << "\" already exists, but --overwrite has not been set. "
                                              "Cannot produce output. Quitting..."
                     << std::ends;
...
int      main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (vm.count("output")) {
            hdp::redirect("output", hdp::out, std::cout);
            hdp::out_redirected = true;
        }
        if (vm.count("log")) {
            hdp::redirect("log", hdp::log, std::clog);
            hdp::log_redirected = true;
        }
