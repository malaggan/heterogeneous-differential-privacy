...
#include <type_traits>
#define maybe_lift_arith(op)                                           \
    template<typename T1, typename T2>                                 \
    std::enable_if_t<                                                  \
            std::is_arithmetic_v<T1> and                               \
                    std::is_arithmetic_v<T2>,                          \
            maybe<decltype(std::declval<T1>() op std::declval<T2>())>> \
    operator op(maybe<T1> a, maybe<T2> b) {                            \
        if (a && b)                                                    \
